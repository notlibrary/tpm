...
I started coding it when found 3 different toothpaste tubes in the bathroom still using it without an issue
...
- And finally it's the tiny size less than 52KB with `-Os` flag almost 1.25 times smaller than LMDB (Windows .exe however has 200KB builtin .ico)
...
that's all is up to you while my business here is almost done(I am Tom Bombadil like character 
I hold it for 5 minutes then gave you the whole thing back and quit because enough and do not need it at all)
...
controlled by TCL script Basically backend simulates toothpaste production process from formulation to batches
and everything else that happening between in the global toothpaste company

This is an ongoing effort "when it's done" 

To ensure authenticity in the high end special cases this recommendations must be digitally signed by production manager dentist or lab researcher 
